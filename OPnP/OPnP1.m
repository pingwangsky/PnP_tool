function [R0 t0 error0 flag] = OPnP(U,u,label_polish)
...
[xx yy zz tt] = GB_Solver_3Order_4Variable_Symmetry(coeff1, coeff2, coeff3, coeff4);
...
if strcmp(label_polish,'polish')
    for jj = 1:length(xx)
        [xx(jj) yy(jj) zz(jj) tt(jj) label_psd(jj)] = PnP_Polish(Q,q,[xx(jj) yy(jj) zz(jj) tt(jj)]);
    end
end
...
for i = 1:length(xx)    
    ...
    Rot(:,:,index) = R; trans(:,index) = t; error(index) = err; %error1(index) = err_proj;
    index = index + 1;
end
...
for i=1:size(R0,3)
    XXc= R0(:,:,i)*U+t0(:,i)*ones(1,n);
    xxc= [XXc(1,:)./XXc(3,:); XXc(2,:)./XXc(3,:)];
    err = norm(xxc-u,'fro')/n;
    res{i}.R= R;
    res{i}.t= t;
    res{i}.err= err;
end
